...
static GLuint CompileShader(GLenum shader_type, const char *data) {
  ...
  LOG("shader source loaded\n");
  glCompileShader(shader_id);
  LOG("shader source compiled\n");
...
class Shader {
 public:
  ...
  Shader& SetFragmentShader(const char *filename) {
    char *data = LoadShader(filename);
    fragment_shader_id_ = CompileShader(GL_FRAGMENT_SHADER, data);
    HandleGLError(__func__);
    free(data);
